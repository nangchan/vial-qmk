...
report_mouse_t pointing_device_task_user(report_mouse_t mouse_report) {
    ...
    // Handle hardware zoom gestures - OS-aware zoom commands (if enabled)
    ...
    if (zoom_enabled && (mouse_report.buttons & (1 << 6)) != 0) { // Mouse Button 7 - Zoom Out
        ...
        if (!zoom_active) {
            if (detected_os == OS_MACOS || detected_os == OS_IOS) {
                tap_code16(G(KC_MINS)); // Cmd+Minus on macOS/iOS
            } else {
                tap_code16(C(KC_MINS)); // Ctrl+Minus on Linux/Windows/Default (universal)
            }
            zoom_active = true;
        }
        ...
        if (!zoom_active) {
            if (detected_os == OS_MACOS || detected_os == OS_IOS) {
                tap_code16(G(KC_PLUS)); // Cmd+Plus on macOS/iOS
            } else {
                tap_code16(C(KC_PLUS)); // Ctrl+Plus on Linux/Windows/Default (universal)
            }
            zoom_active = true;
        }
